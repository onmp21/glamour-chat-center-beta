...
import { Loader2, FileText, BarChart, Lightbulb, Download, MessageSquare } from 'lucide-react';
import { format } from 'date-fns';
import { ptBR } from 'date-fns/locale';

interface ConversationReportGeneratorProps {
  channelId: string;
  channelName: string;
  sessionId: string;
  contactName: string;
...
export const ConversationReportGenerator: React.FC<ConversationReportGeneratorProps> = ({
  channelId,
  channelName,
  sessionId,
  contactName,
  ...
  const { loading, error, reportResult, generateConversationReport } = useEnhancedReports();
  const [activeTab, setActiveTab] = useState('report');

  const handleGenerateReport = async () => {
    await generateConversationReport(channelId, sessionId, period);
  };
  ...
  const handleDownloadPDF = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>Relatório de Conversa - ${contactName}</title>
        <meta charset="utf-8">
      ...
      <body>
        <div class="header">
          <h1>Relatório de Conversa - ${contactName}</h1>
        ...
        <Card className={isDarkMode ? 'bg-card border-border' : 'bg-white border-gray-200'}>
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              ...
              Gerar Relatório de Conversa
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-medium mb-1">Canal</h3>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold">
              Relatório de Conversa - {contactName}
            </h2>
            <Button
              variant="outline"
