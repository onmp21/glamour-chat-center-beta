...
export const ChannelCard: React.FC<ChannelCardProps> = ({
  ...
  if (loading) {
    return <Card className={cn("cursor-pointer transition-all hover:shadow-md animate-pulse", isDarkMode ? "bg-[#18181b] border-[#3f3f46]" : "bg-white border-gray-200")}>
        <CardContent className="p-4">
          <div className="h-20 bg-gray-300 rounded"></div>
        </CardContent>
      </Card>;
  }
  return <Card className={cn("cursor-pointer transition-all hover:shadow-md hover:scale-[1.01] relative group", isDarkMode ? "bg-[#18181b] border-[#3f3f46] hover:bg-[#27272a]" : "bg-white border-gray-200 hover:bg-gray-50", channelIsPinned && "ring-2 ring-[#b5103c]/30")} onClick={() => onClick(channelId)}>
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className={cn("p-2 rounded-md relative", isDarkMode ? "bg-[#27272a]" : "bg-gray-100")}>
              <IconComponent size={18} className="text-[#b5103c]" />
              {unreadNotifications > 0 && <div className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center font-medium">
          ...
          <div className="flex items-center gap-2">
            {counts.total > 0 && <Badge className="text-white text-xs bg-zinc-900 px-[2px] py-0">
                {counts.total}
              </Badge>}
            <button onClick={e => {
            e.stopPropagation();
            togglePin(channelId);
          }} className={cn("p-1 rounded-md", channelIsPinned && "opacity-100", isDarkMode ? "hover:bg-zinc-700" : "hover:bg-gray-200")}>
              <Pin size={14} className={cn(channelIsPinned ? "text-[#b5103c] fill-current" : isDarkMode ? "text-gray-400" : "text-gray-600")} />
            </button>
          </div>
        </div>
