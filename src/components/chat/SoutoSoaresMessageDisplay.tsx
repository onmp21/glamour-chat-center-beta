...
interface SoutoSoaresMessageDisplayProps {
  message: {
    id: string;
    content: string;
    timestamp: string;
...
export const SoutoSoaresMessageDisplay: React.FC<SoutoSoaresMessageDisplayProps> = ({ 
  message, 
  isDarkMode 
}) => {
  const isAgent = 
    message.tipo_remetente === 'USUARIO_INTERNO' || 
    message.tipo_remetente === 'SoutoSoares-ai' ||
    message.sender === 'agent' || 
    message.isOwn;
  
  const displayName = isAgent 
    ? (message.agentName || 'Souto Soares') 
