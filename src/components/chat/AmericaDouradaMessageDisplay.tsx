...
interface AmericaDouradaMessageDisplayProps {
  message: {
    id: string;
    content: string;
    timestamp: string;
...
export const AmericaDouradaMessageDisplay: React.FC<AmericaDouradaMessageDisplayProps> = ({
  message,
  isDarkMode,
  channelName = 'América Dourada',
  userName
}) => {
  const isAgent =
    message.tipo_remetente === 'USUARIO_INTERNO' ||
    message.tipo_remetente === 'America-Dourada' ||
    message.sender === 'agent' ||
    message.isOwn;

  const displayName = isAgent
    ? (message.agentName || 'América Dourada')
