...
export const ExamesTable: React.FC<ExamesTableProps> = ({
  ...
  const filteredExams = exams.filter(exam => {
    const matchesSearch = exam.name.toLowerCase().includes(searchTerm.toLowerCase()) || exam.phone.includes(searchTerm) || exam.instagram && exam.instagram.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesCity = selectedCity === 'all' || exam.city === selectedCity || selectedCity === 'America Dourada' && (exam.city.toLowerCase().includes('amarica') || exam.city.toLowerCase().includes('america'));
  ...
  if (loading) {
    return <div className={cn("flex-1 flex items-center justify-center", isDarkMode ? "bg-background" : "bg-gray-50")}>
  ...
  return <div className={cn("flex-1 overflow-auto p-4", isDarkMode ? "bg-background" : "bg-gray-50")}>
      <div className="space-y-6">
        ...
        <div className={cn("p-4 border rounded-lg", isDarkMode ? "border-border bg-card" : "border-gray-200 bg-white")}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <Button onClick={() => setShowThisWeek(!showThisWeek)} variant={showThisWeek ? "default" : "outline"} className="flex items-center gap-2">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="relative flex-1 max-w-md">
              ...
              <Input placeholder="Buscar por nome, telefone ou Instagram..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
            </div>
            
            <div className="flex gap-2 flex-wrap">
              <Button variant={selectedCity === 'all' ? 'default' : 'outline'} onClick={() => setSelectedCity('all')}>
                Todas as Cidades
              </Button>
              {cities.map(city => <Button key={city} variant={selectedCity === city ? 'default' : 'outline'} onClick={() => setSelectedCity(city)}>
                  {city}
                </Button>)}
            </div>
          </div>
        ...
        <div className={cn("border rounded-lg overflow-hidden", isDarkMode ? "border-border" : "border-gray-200")}>
          <Table>
            <TableHeader>
              <TableRow className={cn(isDarkMode ? "bg-muted/50 border-border" : "bg-gray-50 border-gray-200")}>
                <TableHead className="w-12">
                  <Checkbox checked={filteredExams.length > 0 && selectedExams.length === filteredExams.length} onCheckedChange={handleSelectAll} />
                </TableHead>
            ...
            <TableBody>
              {filteredExams.map(exam => <TableRow key={exam.id} className={cn("transition-colors", selectedExams.includes(exam.id) && "bg-primary/10", isDarkMode ? "hover:bg-muted/50 border-border" : "hover:bg-gray-50 border-gray-200")}>
                  <TableCell>
                    <Checkbox checked={selectedExams.includes(exam.id)} onCheckedChange={() => handleExamSelect(exam.id)} />
                  </TableCell>
                  ...
                  <TableCell className={cn(isDarkMode ? "text-foreground" : "text-gray-900")}>
                    {exam.city.toLowerCase().includes('amarica') || exam.city.toLowerCase().includes('america') ? 'America Dourada' : exam.city}
                  </TableCell>
                  <TableCell>
                    <Button size="sm" variant="outline" onClick={() => handleEdit(exam)} className="h-8 w-8 p-0">
          ...
          {filteredExams.length === 0 && <div className={cn("text-center py-8", isDarkMode ? "text-muted-foreground" : "text-gray-500")}>
