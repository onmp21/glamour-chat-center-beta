...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔄 [MIGRATE_BASE64] Starting migration for tables: ${tablesToMigrate.join(', ')}`);
    ...
    for (const tableName of tablesToMigrate) {
      ...
      if (fetchError) {
        console.error(`❌ [MIGRATE_BASE64] Error fetching from ${tableName}:`, fetchError);
      ...
      for (const message of messages) {
        try {
          ...
          if (uploadError) {
            console.error(`❌ [MIGRATE_BASE64] Upload error for message ${message.id}:`, uploadError);
            continue;
          }
          ...
          if (updateError) {
            console.error(`❌ [MIGRATE_BASE64] Update error for message ${message.id}:`, updateError);
            continue;
          }
