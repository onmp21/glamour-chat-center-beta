...
serve(async (req) => {
  ...
  try {
    ...
    console.log('📥 [WEBHOOK_SOUTO_SOARES] Received webhook:', JSON.stringify(payload, null, 2));

    if (!payload.data || !payload.data.key || !payload.data.message) {
      console.log('❌ [WEBHOOK_SOUTO_SOARES] Invalid message format');
      return new Response(
    ...
    if (message.message.conversation) {
      ...
      if (messageType !== 'text') {
        ...
        if (mediaUrl) {
          console.log(`🎬 [WEBHOOK_SOUTO_SOARES] Processing ${messageType} media from URL`);
          ...
          if (base64Content) {
            ...
            console.log(`✅ [WEBHOOK_SOUTO_SOARES] Media converted to base64 successfully`);
          } else {
            messageContent = getMediaCaptionFromMessage(message.message, messageType);
            console.log(`⚠️ [WEBHOOK_SOUTO_SOARES] Failed to download media, using caption/placeholder`);
          }
        } else {
          messageContent = getMediaCaptionFromMessage(message.message, messageType);
        }
    ...
    if (message.key.fromMe) {
      ...
      console.log(`📤 [WEBHOOK_SOUTO_SOARES] Processing outgoing message from Souto Soares to ${phoneNumber}`);
    } else {
      contactName = message.pushName || phoneNumber;
      tipoRemetente = 'CONTATO_EXTERNO';
      console.log(`📥 [WEBHOOK_SOUTO_SOARES] Processing incoming message from ${phoneNumber} (${contactName})`);
    }
    
    const sessionId = phoneNumber;
    const timestamp = new Date(message.messageTimestamp * 1000).toISOString();

    console.log(`💾 [WEBHOOK_SOUTO_SOARES] Saving ${tipoRemetente} message type ${messageType}: ${messageType === 'text' ? messageContent.substring(0, 50) : '[MEDIA]'}...`);
    ...
    if (error) {
      console.error('❌ [WEBHOOK_SOUTO_SOARES] Error saving message:', error);
      return new Response(
    ...
    console.log('✅ [WEBHOOK_SOUTO_SOARES] Message saved successfully');
    ...
    console.error('❌ [WEBHOOK_SOUTO_SOARES] Error processing webhook:', error);
    return new Response(
