...
function extractPhoneAndName(messageData) {
  ...
  if (typeof messageData?.pushName === 'string' && messageData.pushName.trim() && !messageData?.key?.fromMe) {
    ...
    name = typeof messageData.agentName === 'string'
        ? messageData.agentName.trim() || 'GerenteExterno'
        : 'GerenteExterno';
  }
  return { phone, name };
}
...
const TABLE_NAME = "gerente_externo_conversas";
const LOG_PREFIX = "[Webhook-GerenteExterno]";
...
async function processMessage(supabase, messageData, tableName, instance) {
  try {
    ...
    let realMessageContent = getMessageContent(messageData);
    ...
    if (usedMediaUrl) {
      ...
      console.log(`[Webhook-GerenteExterno] Salva mídia como DataURL:`, (mediaBase64+"").substring(0,60),'...');
    } else if (mediaUrl) {
      mediaBase64 = null;
