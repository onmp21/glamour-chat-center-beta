...
async function processMessage(supabase, messageData, tableName, instance) {
  try {
    ...
    const checkedMediaUrl = toDataUrlIfBase64(mediaUrl);
    const usedMediaUrl = checkedMediaUrl && isDataUrl(checkedMediaUrl) ? checkedMediaUrl : null;

    if (usedMediaUrl) {
      mediaBase64 = usedMediaUrl;
      realMessageContent = placeholder || '[Mídia]';
      console.log(`[Webhook-America] Salva mídia como DataURL:`, (mediaBase64+"").substring(0,60),'...');
    } else if (mediaUrl) {
      mediaBase64 = null;
