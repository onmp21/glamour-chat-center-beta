...
function extractPhoneAndName(messageData: any) {
  ...
  if (typeof messageData?.pushName === 'string' && messageData.pushName.trim() && !messageData?.key?.fromMe) {
    ...
    name = typeof messageData.agentName === 'string'
        ? messageData.agentName.trim() || 'GerenteLojas'
        : 'GerenteLojas';
  }
  return { phone, name };
}
...
const TABLE_NAME = "gerente_lojas_conversas";
const LOG_PREFIX = "[Webhook-GerenteLojas]";
...
async function processMessage(supabase, messageData, tableName, instance) {
  try {
    ...
    if (usedMediaUrl) {
      ...
      console.log(`[Webhook-GerenteLojas] Salva mídia como DataURL:`, (mediaBase64+"").substring(0,60),'...');
    } else if (mediaUrl) {
      mediaBase64 = null;
